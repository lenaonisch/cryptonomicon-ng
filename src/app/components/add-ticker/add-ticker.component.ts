...
export class AddTickerComponent implements OnInit {
  ...
  @Output() awailableTickersLoaded = new EventEmitter();
  @ContentChildren(ConditionalAdvertComponent)
  content!: ConditionalAdvertComponent;
  ...
  ngOnInit(): void {
    this.tickerService
      .fetchAwailableCoins()
      .subscribe(
        (response) => (this.availableCoins = Object.keys(response.Data))
      );
  }

  addTickerHandler(newName = this.ticker) {
    newName = newName.toUpperCase();
    this.isTickerAwailable =
      this.availableCoins.findIndex((t) => t === newName) > 0;
